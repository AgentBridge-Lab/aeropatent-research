...
function familyKey(row) {
  return row.family_id && String(row.family_id) !== "0" ? String(row.family_id) : `publication:${row.publication_number}`;
}
...
async function main() {
  ...
  const reportSnapshot = argValue("--report-from-snapshot", null);
  if (reportSnapshot) {
    // Offline documentation refresh: preserve all cached counts and snapshot dates.
    const landscape = loadJson(reportSnapshot);
    fs.mkdirSync(reportsDir, { recursive: true });
    writeReport(path.join(reportsDir, "bq_landscape_report_ko.md"), landscape);
    console.log(`Refreshed report from preserved snapshot ${sourcePath(reportSnapshot)}`);
    return;
  }
  ...
  const now = new Date();
  const explicitAnalysisDate = argValue("--analysis-date", null);
  const analysisDate = explicitAnalysisDate ? Number(explicitAnalysisDate.replaceAll("-", "")) : dateIntInTimeZone(now, ANALYSIS_TIME_ZONE);
  const parsedDate = new Date(`${String(analysisDate).slice(0, 4)}-${String(analysisDate).slice(4, 6)}-${String(analysisDate).slice(6, 8)}T00:00:00Z`);
  if (!/^\d{8}$/.test(String(analysisDate)) || Number.isNaN(parsedDate.valueOf()) || dateIntInTimeZone(parsedDate, "UTC") !== analysisDate)
    throw new Error("--analysis-date must be a valid YYYY-MM-DD or YYYYMMDD date");
  ...
  const landscape = {
    ...
    source: {
      ...
      collectionDate: argValue("--collection-date", null),
      collectionDateStatus: argValue("--collection-date", null) ? "caller_supplied" : "unknown_not_inferred_from_aggregation_time",
      note: "CPC-prefix candidates, not text-validated aerospace patents. Family metrics use distinct nonzero family_id, with missing IDs counted separately by publication. Office/field/year memberships overlap. Applicant names retain source spelling (first five names per publication in collection SQL); CPC counts are row-prefix occurrences (first twenty source codes), not family shares.",
    },
    summary: {
      rowCount,
...
집계 산출물 생성시각: ${landscape.generatedAt}
집계기준일: ${summary.analysisDate ?? "미기록"}
수집일: ${landscape.source?.collectionDate ?? "미확인 — 집계 생성시각을 수집일로 사용하지 않음"}
...
- 분야-공보 행 (분야 간 중복 포함): ${summary.rowCount.toLocaleString()} rows
