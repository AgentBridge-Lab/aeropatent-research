-- Production CPC-first candidate finder. No LIMIT. Use only after dry-run
-- review, and consider splitting by field/region/month when needed.
WITH field_taxonomy AS (
  SELECT 'space_launch_propulsion_recovery' AS field_id, ['B64G1','F02K9','F02K99'] AS cpc_prefixes
  UNION ALL SELECT 'space_satellite_bus_thermal_power', ['B64G1','H01L31','F28D15']
  UNION ALL SELECT 'space_comm_leo_network', ['H04B7','H04W84','H04L45']
  UNION ALL SELECT 'space_remote_sensing_payload', ['G01S13','G01S7','G01J3']
  UNION ALL SELECT 'space_gnc_rendezvous_servicing', ['B64G1','G05D1','G01C21']
  UNION ALL SELECT 'space_materials_tps_coatings', ['C04B35','C09D5','B32B']
  UNION ALL SELECT 'aviation_propulsion_sustainable', ['B64D27','B64D33','F02C7','F02K3']
  UNION ALL SELECT 'aviation_structures_aero_composites', ['B64C3','B64C21','B64C23','B29C70']
  UNION ALL SELECT 'aviation_avionics_flight_control_autonomy', ['G05D1','B64C13','G08G5','G01C21']
),
base AS (
  SELECT
    publication_number,
    country_code AS publication_country_code,
    family_id,
    publication_date,
    priority_date,
    grant_date,
    assignee_harmonized,
    cpc
  FROM `patents-public-data.patents.publications`
  WHERE priority_date BETWEEN CAST(FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL 10 YEAR)) AS INT64)
      AND CAST(FORMAT_DATE('%Y%m%d', CURRENT_DATE()) AS INT64)
)
SELECT
  b.publication_number,
  b.publication_country_code,
  b.family_id,
  b.publication_date,
  b.priority_date,
  b.grant_date,
  f.field_id,
  ARRAY(SELECT AS STRUCT name, country_code FROM UNNEST(b.assignee_harmonized) LIMIT 5) AS assignees,
  ARRAY(SELECT DISTINCT country_code FROM UNNEST(b.assignee_harmonized) WHERE country_code IS NOT NULL) AS assignee_country_codes,
  ARRAY(SELECT code FROM UNNEST(b.cpc) LIMIT 20) AS cpc_codes,
  1 AS cpc_hit
FROM base b
CROSS JOIN field_taxonomy f
WHERE EXISTS (
  SELECT 1
  FROM UNNEST(b.cpc) c
  JOIN UNNEST(f.cpc_prefixes) prefix
  ON STARTS_WITH(c.code, prefix)
);
