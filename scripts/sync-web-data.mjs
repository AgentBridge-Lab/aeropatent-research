...
// 본체 집계 화면(분석·국가 비교·보고서)은 전체 기간 고정 기준을 사용한다.
...
// ---------------------------------------------------------------------------
// 본체(BigQuery 실측 집계) — 전체 기간 고정 기준. 비례 배분·분할 없음.
// ---------------------------------------------------------------------------
...
export interface Summary {
  total_patents: number; // 고유 패밀리, 전체 기간
  publication_count: number;
  leading_country: CountryCode; // 표시 5개 공개 관할 중 최대
  field_count: number;
  country_distribution: CountryDist[]; // 전체 기간 · 공개 관할 기준
  yearly_trend: YearPoint[]; // 전체 후보군 연도별 실측 (문헌 행 우선연도 기준)
  field_heatmap: HeatCell[]; // 전체 기간 실측 country-family counts
  insights: string[];
}
...
export function getSummary(): Summary {
  ...
  return {
    ...
    insights: [
      \`전체 기간 고유 패밀리는 \${LANDSCAPE_SUMMARY.family_count.toLocaleString()}개, 공개 문헌은 \${LANDSCAPE_SUMMARY.publication_count.toLocaleString()}건입니다 (\${CANDIDATE_SCOPE_NOTE}).\`,
...
export interface FieldAnalysis {
  ...
  country_distribution: CountryDist[]; // 전체 기간 실측
...
export function getFieldAnalysis(fieldId: FieldId): FieldAnalysis | null {
  ...
  return {
    field,
    one_line: field.report_bullets[0] ?? \`\${field.label_ko} 분야는 전체 기간 \${field.family_count.toLocaleString()}개 패밀리 규모입니다 (\${CANDIDATE_SCOPE_NOTE}).\`,
...
export interface CountryProfile {
  country: Country;
  total: number; // 전체 기간 공개 관할 기준 패밀리 (실측)
  strong_fields: { field: Field; count: number }[]; // 전체 기간 실측
  top_patents: Patent[]; // 표본 문헌
  vs_korea: string;
}
    ...
    return {
      ...
      vs_korea:
        code === 'KR'
          ...
          : \`\${country.label_ko} 공개 관할의 \${lead?.field.label_ko ?? ''} 패밀리는 \${(lead?.count ?? 0).toLocaleString()}개, 같은 분야 KR 공개 관할은 \${krLeadCount.toLocaleString()}개입니다 (전체 기간 · 공개 관할 기준).\`,
    };
  });
