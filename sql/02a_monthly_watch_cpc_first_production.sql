-- Production monthly monitoring query using CPC first. No LIMIT.
...
SELECT
  ...
  ARRAY(SELECT DISTINCT country_code FROM UNNEST(r.assignee_harmonized) WHERE country_code IS NOT NULL) AS assignee_country_codes,
  ARRAY(SELECT code FROM UNNEST(r.cpc) LIMIT 20) AS cpc_codes
FROM recent r
